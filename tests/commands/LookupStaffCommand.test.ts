...
describe("LookupStaffCommand", () => {
  ...
  staffService = new StaffService(
    mockStaffRepo,
    mockRedeemRepo
  ) as jest.Mocked<StaffService>;
  let lookupStaffCommand: LookupStaffCommand = new LookupStaffCommand(
    staffPassId,
    staffService
  );

  it("execute returns correct result when staff exists", async () => {
    const mockResDTO: CommandResDTO = {
      resultMessage:
        "lookup-staff done: \n" +
        "Staff Record found:\n" +
  ...
  it("execute throws CommandExecutionError when ServiceError occurs", async () => {
    staffService.lookUpStaffByPassId.mockRejectedValue(
      new ServiceError("Service error message")
    );

    await expect(lookupStaffCommand.execute()).rejects.toThrow(
